...
import darkcarnival from './darkcarnival_token';
import dc_token_url from './darkcarnival_token_url';
import dota2_heros from './dota2_heros_with_traits';

// 构建中文名 → 英文名 映射
const nameMap = {};
for (const [engKey, hero] of Object.entries(dota2_heros)) {
    nameMap[hero.chi_name] = engKey;
}

// 使用示例：通过中文名获取英雄数据
function getHeroByChineseName(chiName) {
    const engKey = nameMap[chiName];
    return engKey ? dota2_heros[engKey] : undefined;
}

// 测试
const hero = getHeroByChineseName('敌法师');
console.log(hero.darkcarnival); // 输出 ['magician','high_priestess','emperor']

/////////////主逻辑//////////////////////////////////////
export default function initScript() {
    //console.log(darkcarnival);
    //console.log(dota2_heros);
    //console.log(dc_token_url);
    new BaseClass();
}
...
function showHeroByToken(herolist) {
    ...
    for (let i = 0; i < heroElements2.snapshotLength; i++) {
        ...
        const hero = getHeroByChineseName(name);
    ...
    if (grayimg == '') {
      ...
      */
        ...
        if (herolist == undefined) {
            ...
            var tokendiv = document.getElementById('tokendiv_' + hero.name);
            if (tokendiv) {
                tokendiv.style.opacity = 1; // 透明度を50%に指定
            } else {
                console.log(hero.name, ibox[0]);
            }
        } else if (herolist.indexOf(hero.name) >= 0) {
            // 展示匹配代币的英雄
            //console.log(name);
            ibox[0].style.opacity = 1; // 透明度を50%に指定
            var tokendiv = document.getElementById('tokendiv_' + hero.name);
...
function hitSearchKey(searchKeys) {
    ...
    if (searchKeys && searchKeys.length > 0) {
        ...
        Object.keys(darkcarnival).forEach(function (key) {
            if (terminate) {
                return;
            }
            ...
            if (searchKeys.indexOf(key) >= 0) {
                ...
                let tmp = [];
                Object.keys(darkcarnival[key]).forEach(function (key2) {
                    tmp = tmp.concat(darkcarnival[key][key2]);
                });
...
function resourceLoaded(todoFunc) {
    ...
    Object.keys(dc_token_url).forEach(function (key) {
        // 触发网络请求 让浏览器缓存图片
        // https://juejin.cn/post/7340167256267391012
        const img = new Image();
        img.alt = key;
        img.title = dc_token_url[key].name;
        img.onerror = () => {
            // 图片加载失败则重试
            img.src = dc_token_url[key].url;
        ...
        img.onload = () => {
            ...
            if (loadCount >= Object.keys(dc_token_url).length) {
                flagResLoaded = true;
                todoFunc();
            }
        };
...
function renderTokenButtonList() {
    ...
    Object.keys(dc_token_url).forEach(function (key) {
        ...
        tokennode_input.id = 'input-' + key;
        tokennode_input.style.backgroundImage = "url('" + dc_token_url[key].url + "')";
        ...
        tokennode_text.innerHTML = dc_token_url[key].name;

        tokennode.appendChild(tokennode_inputdiv);
        tokennode.appendChild(tokennode_text);

        //console.log('img', dc_token_url[key].url, dc_token_url[key].name);

        tokenlistdiv.appendChild(tokennode);
        //console.log(key, dc_token_url[key]);
    });
    ...
    for (var i = 0; i < inputTag.length; i++) {
        if (inputTag[i].id.indexOf('input-') >= 0) {
        ...
        tokenTag[i].addEventListener('change', function () {
            ...
            for (var i = 0; i < tokenTag.length; i++) {
                if (tokenTag[i].checked) {
                    console.log(tokenTag[i].id, tokenTag[i].id.split('-')[1]);
                    tmpoutput.push(tokenTag[i].id.split('-')[1]);
                }
            }
...
let mainPage =
    'https://greasyfork.org/zh-CN/scripts/501114-dota2-%E5%80%BE%E5%A4%A9%E4%B9%8B%E6%88%98%E7%AC%AC%E4%B8%89%E5%B9%95-%E5%86%B0%E5%B7%9D%E6%AE%8B%E9%AA%B8-%E6%A0%BC%E7%BD%97%E5%BE%B7%E5%9B%BE%E4%B9%A6%E9%A6%86-%E4%BB%A3%E5%B8%81%E7%AD%9B%E9%80%89%E8%8B%B1%E9%9B%84';
let updateURL =
    'https://update.greasyfork.org/scripts/501114/dota2%20%E5%80%BE%E5%A4%A9%E4%B9%8B%E6%88%98%E7%AC%AC%E4%B8%89%E5%B9%95%20%E5%86%B0%E5%B7%9D%E6%AE%8B%E9%AA%B8%20%E6%A0%BC%E7%BD%97%E5%BE%B7%E5%9B%BE%E4%B9%A6%E9%A6%86%20%E4%BB%A3%E5%B8%81%E7%AD%9B%E9%80%89%E8%8B%B1%E9%9B%84.user.js';
