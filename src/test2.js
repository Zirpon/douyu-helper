...
(function () {
    ...
    function btnListFun(localStorageName, displayName, description) {
        $('#wah0713').append(
            `<button class='${localStorageName}' title='${description}'>${displayName}(close)</button>`
        );
    ...
    const defaultConfig = {
        ...
        isShowRoomData: {
            name: '显示房间数据',
            description:
                '显示房间数据（时间范围今天00:00到今晚24:00）,12分钟刷新数据一次__本功能由【BerryBarry11】提出',
            value: true,
            firstDelayTime: 10 * 1000,
            action: (value) => {
    ...
    const config = new Proxy(
        {},
        {
            set: function (target, propKey, value, receiver) {
                const { firstDelayTime, action } = defaultConfig[propKey];
    ...
    for (let key in defaultConfig) {
        const { name, description, value } = defaultConfig[key];
    ...
    const observer = new MutationObserver(function () {
        ...
        if (config.playerBottomSimplification) {
            ...
            $(
                '.ActivityItem:not(.ActivityItem[data-flag="room_level"]):not(.ActivityItem[data-flag="anchor_quiz"])'
            ).addClass('is-hide');
        ...
        if (config.backgroundIsShow && !$('.is-fullScreenPage').length) {
            ...
            !originalLayoutMainMarginTop &&
                (originalLayoutMainMarginTop = $layoutMain.css('margin-top').split('px')[0] - 0);
            if (once.backgroundIsShow) {
                setTimeout(() => {
        ...
        // 输入框上方送礼3000毫米淡出
        $('#js-player-barrage .BarrageBanner')
            .children()
            .delay(1000 * 3)
            .fadeOut('slow');
    ...
    async function showRoomData() {
        ...
        const firstRow = `💬弹幕数:${formatData(chatPv)}👨‍👩‍👧‍👦发弹幕人数:${formatData(chatUv)}⏱️直播时间:${formatData(
            onlineMinutes
        )}分🔥活跃人数:${formatData(activeUv)}`;
        const secondRow = `🎁礼物价值:${formatPrice(giftAllPrice)}元🎅🏻礼物送礼人数:${formatData(
            giftAllUv
        )}💸付费礼物:${formatPrice(giftPaidPrice)}元🤴🏻付费送礼人数:${formatData(giftPaidUv)}
      `;
